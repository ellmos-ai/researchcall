...
This file separates the operator-provided live measurements in `FINDINGS.md` from commands actually executed in this repository during the post-findings run. This run made no real call and no CALL-E network request. Since the public privacy remediation on 2026-08-24, all concrete phone numbers, people, service identifiers, answers, timestamps, and transcript excerpts in public evidence are explicitly synthetic reconstructions; measured dates, aggregate outcomes, commands, and test counts remain factual.
...
Derived from the private live record, then rewritten here as a non-identifying summary:
...
Migration checked against a private local copy that was never tracked: the
...
Derived from private live records, then rewritten here as a non-identifying summary:
