#!/usr/bin/env python3
"""Package, deploy, and tear down the ``calle-demo-researchcall`` Lambda Function URL.

Plain boto3 + zipfile, following the same pattern already deployed for `roshambo`
and its `ringedingeding` sibling (``ringedingeding/infra/deploy_demo_lambda.py``)
-- no SAM/CDK/CloudFormation. This project's storage is a JSON workspace file
plus, once a field phase is prepared, stdlib SQLite -- no external database, no
TLS root certificate to bundle, and no ``uvicorn`` either -- Mangum replaces it.

Subcommands
-----------

``package``      Build ``calle-demo-researchcall.zip``: the ``researchcall``
                 package (from ``src/researchcall``), ``demo/`` (the Mangum
                 adapter), ``pipeline/`` (the ``*.forms.yaml`` definitions
                 ``researchcall.forms.load_fields`` reads -- see
                 ``demo/lambda_entry.py::_pipeline_root`` for why it has to be
                 packaged as an explicit sibling directory), and the runtime
                 dependencies as Lambda-compatible (manylinux) wheels
                 regardless of the platform this script runs on.
``create-role``  Create or update the execution role. **Not** the managed
                 ``AWSLambdaBasicExecutionRole`` (that policy's ``Resource`` is
                 ``*`` -- every log group in the account) -- an inline policy
                 scoped to exactly this function's own log group, nothing
                 else. This function needs no other AWS permission: no
                 Bedrock, no S3, no database access, because it has none.
``deploy``       Create or update the Lambda function from a packaged zip,
                 with ``ReservedConcurrentExecutions=1`` (see "Cost guard")
                 and a short ``--timeout`` (default 15s).
``enable-url``   Create the public Function URL (``AuthType=NONE``) and the
                 matching resource-policy statements that actually allow
                 anonymous invocation.
``teardown``     Best-effort delete of the function, its Function URL, its
                 role's inline policy and the role itself, and its log group.
                 Not a transactional stack deletion.

Live-path hardening -- the point this deployment exists to prove
------------------------------------------------------------------

Two independent things make it structurally impossible for this deployment to
place a real telephone call, and both are enforced here, not left to a
comment:

1. ``_demo_environment()`` returns **exactly** ``{"DEMO_MODE": "1"}`` -- no
   ``CALLE_*`` variable is ever read from the deploying shell or passed
   through. There is no passthrough mechanism in this file at all; a key that
   is never set cannot be spent.
2. ``DEMO_MODE=1`` is what actually blocks a call, at the code layer:
   ``researchcall.calls.LiveCallClient.__init__`` checks it before its own
   api-key check and refuses unconditionally
   (``tests/test_live_guard.py``, "demo mode", including a test that
   monkeypatches ``urllib.request.urlopen`` to explode and asserts it is
   never called). Losing (1) alone -- a future refactor that starts passing
   environment variables through -- still leaves (2) standing. A third,
   weaker fact: the web app this Lambda serves has no route that reaches
   ``LiveCallClient`` at all today (see ``demo/lambda_entry.py``'s docstring).

Cost guard
----------

``ReservedConcurrentExecutions=1`` plus AWS Lambda's Free Tier (1M
requests/month) keep an accidental traffic spike inexpensive. A cost guard,
not a security control -- the Function URL itself has no authentication, same
as roshambo's and ringedingeding's; the endpoint being public is the point of
a demo link.
...
PACKAGE_DIR = REPO_ROOT / "src" / "researchcall"
PIPELINE_DIR = REPO_ROOT / "pipeline"
INFRA_DIR = Path(__file__).resolve().parent
BUILD_DIR = INFRA_DIR / "build"
DEFAULT_ZIP_PATH = BUILD_DIR / "calle-demo-researchcall.zip"

FUNCTION_NAME = "calle-demo-researchcall"
ROLE_NAME = "calle-demo-researchcall-role"
HANDLER = "demo.lambda_entry.handler"
DEFAULT_REGION = "eu-central-1"

# mangum has no third-party dependencies of its own; fastapi pulls in
# starlette, pydantic, pydantic-core, anyio, sniffio, idna, typing-extensions
# and annotated-types transitively. Matches the `lambda` extra in
# pyproject.toml -- no uvicorn (Mangum replaces it), no jinja2 (web/render.py
# builds HTML by hand, unlike ringedingeding), pyyaml so
# researchcall.forms._read_entries uses the same parser local dev does
# instead of its small fallback reader.
...
def cmd_package(args: argparse.Namespace) -> None:
    ...
    print(f"Copying {PACKAGE_DIR} -> {build_dir / 'researchcall'}")
    _copy_source_tree(PACKAGE_DIR, build_dir / "researchcall")

    print(f"Copying {DEMO_DIR} -> {build_dir / 'demo'}")
    _copy_source_tree(DEMO_DIR, build_dir / "demo")

    print(f"Copying {PIPELINE_DIR} -> {build_dir / 'pipeline'}")
    _copy_source_tree(PIPELINE_DIR, build_dir / "pipeline")
...
def _copy_source_tree(src: Path, dst: Path) -> None:
    ...
    Bytecode compiled on a different architecture/Python build than the
    Lambda execution environment can be incompatible -- same AWS guidance the
    ringedingeding sibling's identical helper follows.
    """
...
LOG_GROUP_POLICY_NAME = "calle-demo-researchcall-logs"
...
def cmd_create_role(args: argparse.Namespace) -> None:
    ...
    try:
        role = iam.create_role(
            ...
            Description="Execution role for the calle-demo-researchcall Lambda "
...
def cmd_deploy(args: argparse.Namespace) -> None:
    ...
    try:
        response = lam.create_function(
            ...
            Description="researchcall public dry-run demo (CALL-E hackathon). "
    ...
    try:
        ...
        print(
            ...
            "(see module docstring, 'Cost guard')"
        )
    except lam.exceptions.InvalidParameterValueException as exc:
...
def _demo_environment() -> dict[str, str]:
    ...
    ``LiveCallClient.__init__`` checks before anything else.
    """
    return {"DEMO_MODE": "1"}
...
def cmd_teardown(args: argparse.Namespace) -> None:
    ...
    print(
        ...
        "AWS console that no calle-demo-researchcall resources remain billable"
    )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Package, deploy, and tear down the calle-demo-researchcall "
        "Lambda Function URL."
    )
