...
Die sichere Betriebsannahme lautet deshalb: **Art. 50 Abs. 1 und 5 anwenden, bis die ausschließliche Forschungszweck-Ausnahme für die konkrete Bereitstellung schriftlich bestätigt ist.** Seit dem 2026-08-11 belegt der Code diese Anforderung: Die Offenlegung wird nicht mehr aus dem Freitext des Forschenden erwartet, sondern vom Werkzeug gebildet, steht vor jeder Begrüßung und wird wie der Einwilligungssatz gegen das Transkript geprüft. Nicht belegt bleibt, dass der Agent die vorgegebene Reihenfolge einhält — gemessen ist nur, dass er zitierte Sätze wörtlich spricht.
...
- Offenlegung, Umfang und Dauer, ein Datenschutzsatz, Abbruchrecht und Löschung auf Wunsch werden vom Werkzeug gebildet und in jeden Auftrag wörtlich eingesetzt — unabhängig davon, welcher Weg die Studie gebaut hat (`src/researchcall/questionnaire.py`). Die Einleitung und der lange Datenschutzhinweis des Forschers kommen als Öffnungsblöcke dazu; die gibt es nur bei Studien aus der Workbench, ein Fragebogen aus einer Datei trägt keine — deshalb fiel im ersten Feldversuch beides aus (`FINDINGS.md`, Abschnitt 14).
