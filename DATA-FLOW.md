...
| Live call request | Raw selected phone (or, in a field trial with `RESEARCHCALL_FIELD_TRIAL_PHONE`, the one briefed test number instead), exact questionnaire/consent task, result schemas, locale and pseudonymous sample ID metadata | Sent to CALL-E; response is processed in memory before selected fields are persisted | Provider-side retention is not specified in this repository | Participant, CALL-E and CLI process | **Yes, live CLI only:** to the configured URL (default HTTPS); the code does not enforce an HTTPS scheme for overrides | `src/researchcall/calls.py:218-261, 324-367`; `src/researchcall/cli.py:169-194` |
