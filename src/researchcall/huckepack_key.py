...
* **A round outlives its request.** The calls run in a background thread
  (``web/field_phase.py``), and a thread does not inherit a ``ContextVar``. The key is
...
* **The host may have a key of its own.** ``LiveCallClient.from_environment`` reads
  ``CALLE_API_KEY`` from the process environment. In only-host that search must
