...
# -- the workbench file, kept in the same place as the database ------------

#: ResearchCall keeps a second piece of state next to the database: the
#: workbench file ``workspace.json``. In a huckepack mode it must not land on
#: the host's disk either — and it must travel with the export, or a backup
#: would silently restore only half of the work. It therefore lives in a table
#: inside the session database, which makes the snapshot the one artefact that
#: carries everything.
DOCUMENT_TABLE = "huckepack_document"


def session_document(name: str) -> Optional[str]:
    """Read a document from the session database, or ``None`` if unwritten."""
    conn = SESSIONS.connection(current_session() or ANONYMOUS_SESSION)
    conn.execute(
        f"CREATE TABLE IF NOT EXISTS {DOCUMENT_TABLE} "
        "(name TEXT PRIMARY KEY, payload TEXT NOT NULL, written_at TEXT NOT NULL)"
    )
    row = conn.execute(
        f"SELECT payload FROM {DOCUMENT_TABLE} WHERE name = ?", (name,)
    ).fetchone()
    return None if row is None else row[0]


def store_session_document(name: str, payload: str) -> None:
    conn = SESSIONS.connection(current_session() or ANONYMOUS_SESSION)
    conn.execute(
        f"CREATE TABLE IF NOT EXISTS {DOCUMENT_TABLE} "
        "(name TEXT PRIMARY KEY, payload TEXT NOT NULL, written_at TEXT NOT NULL)"
    ...
    conn.execute(
        f"INSERT INTO {DOCUMENT_TABLE} (name, payload, written_at) VALUES (?, ?, ?) "
        "ON CONFLICT(name) DO UPDATE SET payload = excluded.payload, "
        "written_at = excluded.written_at",
        (name, payload, datetime.now(timezone.utc).isoformat(timespec="seconds")),
    )
    conn.commit()
