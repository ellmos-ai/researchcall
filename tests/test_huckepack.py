...
def test_closing_a_connection_does_not_throw_the_session_away(monkeypatch, tmp_path):
    """The data layer closes after every write. For a session that must not delete it."""
    use_mode(monkeypatch, "huckepack-gift")
    reset = huckepack_storage.bind_session(TOKEN)
    try:
        initialize(tmp_path / "study.db")
        ...
        connection.execute(
            "INSERT INTO study (study_key, title, questionnaire_json, created_at) "
            "VALUES ('s1', 'Survives', '{}', '2026-08-02')"
        )
        connection.commit()
        connection.close()  # the data layer does this after every write

        again = connect(tmp_path / "study.db")
        assert [row[0] for row in again.execute("SELECT title FROM study")] == ["Survives"]
    finally:
        huckepack_storage.unbind_session(reset)
